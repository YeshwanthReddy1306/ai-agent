...
export async function generateEscalationContext(leadId: string) {
  ...
  return {
    ...
    lead_score: score,
    conversation_history: messages.rows.map((row: any) => ({
      sender: row.sender,
      content: row.content,
...
async function detectObjections(messages: any[]) {
  const objections = [];
...
async function detectSentiment(messages: any[]) {
...
async function getSuggestedAction(objections: any[], sentiment: string) {
