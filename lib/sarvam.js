  ...
  return withRetry(async () => {
    const res = await fetch(`${SARVAM}/v1/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        // spoken) may override the model. The VOICE path never sets this and always gets
        // the env model (sarvam-105b). Fallback is 105b since sarvam-30b hit EOL 2026-07-26.
...
// Cache primitives, exposed so the STREAMING path can share the same disk cache as the REST
// path. Without these, moving to streaming TTS would silently throw away the ₹0-on-repeat
// cache — and TTS is ~80% of the Sarvam bill (G5 cost ceiling).
function ttsCacheFile(text, lang, emotion, opts = {}) {
  ...
  return path.join(CACHE_DIR, `line-${sig}.b64`);
}
function ttsCacheGet(text, lang, emotion, opts = {}) {
  try { return fs.readFileSync(ttsCacheFile(text, lang, emotion, opts), 'utf8'); } catch { return null; }
}
function ttsCachePut(text, lang, emotion, opts, b64) {
  try { fs.writeFileSync(ttsCacheFile(text, lang, emotion, opts), b64); } catch { /* cache is best-effort */ }
}
...
module.exports = { sttTranscribe, llmChat, ttsSpeak, ttsRaw, ttsCachedLine, ttsCacheGet, ttsCachePut, ackClips, EMOTION_STYLE, TTS_LANGS, serviceHealth };
