...
async function sttTranscribe(wavBuffer) {
  return withRetry(async () => {
    ...
    form.append('language_code', 'unknown'); // auto-detect te/hi/en + 20 more
  ...
  return withRetry(async () => {
    const res = await fetch(`${SARVAM}/v1/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        reasoning_effort: 'low', // live call: speed beats depth
      }),
    });
  ...
  return withRetry(async () => {
    const res = await fetch(`${SARVAM}/text-to-speech`, {
      ...
      body: JSON.stringify({
        ...
        model: 'bulbul:v3',
        speaker: process.env.AGENT_VOICE || 'kavitha',
        pace: style.pace,
        temperature: style.temperature,
        speech_sample_rate: opts.sampleRate || 24000,
...
async function ttsSpeak(text, lang, emotion, opts = {}) {
  ...
  if (clean.length <= 110) return [await ttsRaw(clean, lang, emotion, opts)].filter(Boolean);
...
// Short human sounds played while the LLM thinks. Generated once, cached on disk forever.
const ACK_PHRASES = {
  'te-IN': ['హా…', 'సరే అండి…', 'అలాగా…', 'హ్మ్… ఒక్క నిమిషం అండి…'],
  'hi-IN': ['हाँ…', 'अच्छा…', 'जी जी…', 'हम्म… एक सेकंड…'],
  'en-IN': ['Hmm…', 'Right, right…', 'Okay…', 'One second…'],
};
...
async function ackClips(lang) {
  ...
  for (let i = 0; i < ACK_PHRASES[l].length; i++) {
    const file = path.join(CACHE_DIR, `ack-${l}-${i}.mp3.b64`);
...
module.exports = { sttTranscribe, llmChat, ttsSpeak, ttsRaw, ackClips, EMOTION_STYLE, TTS_LANGS };
