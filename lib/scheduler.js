...
function fromCall(lead, summary) {
  ...
  if (summary.appointment && summary.appointment.booked) {
    ...
    // Record the visit for the counselor to confirm (pilot-safe .ics, no live-calendar double-book).
    try { require('./booking').record(lead, summary.appointment.when); } catch (e) { console.error('[booking] record failed:', e.message); }
  } else if (summary.callbackRequest && summary.callbackRequest.requested) {
    // Parent explicitly asked to be called back at a specific time → honor it. We don't fake
    // precise natural-language time parsing: the automated dueAt is the next high-connect slot,
    // and the parent's own stated time rides in the message so a human/dialer hits it exactly.
    let dueAt = hrs(4);
    try { dueAt = require('./besttime').nextGoodSlot(); } catch { /* fallback to +4h */ }
    schedule({ ...base, type: 'callback_requested', dueAt, channel: 'call',
      message: `Callback requested by ${lead.parentName} (${lead.studentName}) — parent asked for: "${summary.callbackRequest.when}". ${summary.nextAction || ''}` });
