...
function applyRegister(text, lead) {
  ...
  const rules = [
    ...
    [/సరే అండి/g, 'సరేనండి'], [/\bసరే\b/g, 'సరేనండి'], 
    [/అవును అండి/g, 'అవునండి'], [/\bఅవును\b/g, 'అవునండి'], 
    [/లేదు అండి/g, 'లేదండి'], [/\bలేదు\b/g, 'లేదండి'],
...
function formatReminder(personaLang) {
  ...
  return {
    role: 'user',
    content: `SYSTEM REMINDER: You MUST reply ENTIRELY in ${label} (except for English acronyms like IIT/MPC). Reply with 1 or 2 conversational sentences. Maintain your natural, colloquial persona and do NOT become formal or generic. Choose the emotion tag DELIBERATELY from your EMOTION PALETTE — do NOT default to warm. The emotion word goes ONLY inside the tag. MANDATORY: every reply MUST end with the tag ~~${personaLang}|<emotion>~~ as the VERY LAST thing you write. Never forget the tag.`,
  };
}
