...
const EMOTIONS = ['warm', 'excited', 'empathetic', 'calm', 'urgent', 'amused', 'reassuring', 'concerned', 'proud'];
...
// Premortem #3 fix (H1): language-switch hysteresis. The persona flips only after the
// caller stays in a NEW language for 2 consecutive turns — one STT misdetection on
// code-mixed speech can no longer ping-pong the whole call.
// state: { personaLang, streak: { lang, count } }  → returns true if the persona switched.
function nextPersonaLang(state, detected) {
