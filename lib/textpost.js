...
function parseTag(raw, fallbackLang) {
  ...
  while ((m = re.exec(raw)) !== null) {
    // Ignore the LLM's language tag (it can hallucinate due to past history).
    // The language is strictly governed by fallbackLang (call.personaLang).
...
function ttsPhonetics(text, lang) {
  ...
  const rules = lang === 'te-IN' ? [
    ...
    [/\bscholarship\b/gi, 'स्कॉलरशिप'],
    [/\bfee\b/gi, 'फीस'],
    [/\bfees\b/gi, 'फीस'],
    [/\btest\b/gi, 'टेस्ट'],
    [/\bperfect\b/gi, 'परफेक्ट'],
    [/\bwaiver\b/gi, 'वेवर'],
    [/\bregistration\b/gi, 'रजिस्ट्रेशन'],
    [/\bcampus\b/gi, 'कैंपस'],
    [/\bdetails\b/gi, 'डिटेल्स'],
    [/\b10th\b/gi, '10'],
    [/\b12th\b/gi, '12'],
    [/sathvik/gi, 'सात्विक'],
    [/resonance/gi, 'रेजोनेंस'],
    // Strip remaining A-Z letters to avoid TTS hanging on stray English
    [/[a-zA-Z]+/g, '']
  ];
...
function formatReminder(personaLang) {
  ...
  const scriptRule = personaLang === 'hi-IN' ? ' Do NOT use any English Latin letters; transliterate ALL English words, acronyms, and names (like the student or college name) into Devanagari script.' : ' You may use English Latin letters for domain words.';
  return {
    role: 'user',
